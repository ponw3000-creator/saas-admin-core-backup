...
import { roleListMockData, permissionTreeMockData } from '@/mock/index.js'

// [API-Ready] 角色列表数据
// TODO: [API对接] 未来将下面这行替换为: const res = await api.getRoles(); roleList.value = res.data
const roleList = ref([...roleListMockData])
const selectedRoleId = ref(1)
const currentRole = computed(() => roleList.value.find(r => r.id === selectedRoleId.value))
const isCurrentSuperAdmin = computed(() => currentRole.value?.is_super_admin === true)

// [API-Ready] 权限树形结构数据
// TODO: [API对接] 未来将下面这行替换为: const res = await api.getPermissions(); permissionTree.value = res.data
const permissionTree = ref([...permissionTreeMockData])
...
// [API-Ready] 角色与权限 ID 的映射关系（roleId → permissionIds[]）
// TODO: [API对接] 未来将替换为: const res = await api.getRolePermissions(); rolePermissionMap = res.data
const rolePermissionMap = reactive({
  1: getAllPermissionIds(permissionTreeMockData),
  2: [
